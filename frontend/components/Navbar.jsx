...
export default function Navbar() {
  ...
  const getNotificationMeta = (type) => {
    ...
    return {
      ...
      Icon: UsersRound,
      iconClass:
        "bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 ring-cyan-500/15",
    };
  };
        ...
        >
          <div className="max-w-7xl mx-auto flex items-center justify-between w-full px-4 sm:px-6 py-3 sm:py-3.5">
            ...
            <Link href="/" className="flex items-center gap-2 group shrink-0">
              ...
              <span
                className={`font-extrabold text-base sm:text-xl tracking-tight transition-all duration-300 ${
                  !isLoggedIn
                        ...
                        {notifications.length === 0 ? (
                          <div className="px-6 py-9 text-center">
                            ...
                            <p className="mt-1 text-xs text-muted-foreground">
                              New trip, group, and expense updates will appear
                              here.
                            </p>
                          </div>
                        ) : (
                            ...
                            {notifications.slice(0, 8).map((n, i) => {
                              ...
                              return (
                                <li key={n._id || i}>
                                  ...
                                  >
                                    <div
                                      className={`mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ring-1 ${iconClass}`}
                                    >
                                      <Icon size={18} />
                                    </div>
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 px-3 py-3 border-b border-border/50 mb-1">
                          {profile?.avatar || user?.photoURL ? (
                            ...
                            <div
                              className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 border border-cyan-500/20 shadow-sm"
                              style={{
                                background:
                                  "linear-gradient(135deg, #0891B2, #0E7490)",
                              }}
        ...
        <div className="md:hidden fixed bottom-3 inset-x-3 z-50 flex justify-around items-center h-16 rounded-2xl border border-foreground/10 bg-background/70 backdrop-blur-2xl shadow-[0_8px_32px_rgba(8,145,178,0.12)] px-1">
          ...
          {BOTTOM_LINKS.slice(0, 2).map(({ href, icon: Icon, label }) => (
            ...
            >
              ...
              {pathname === href && (
                <span className="w-1 h-1 rounded-full bg-cyan-500 dark:bg-cyan-400" />
              )}
            </Link>
          ))}
          ...
          <div className="relative -top-4 flex flex-col items-center">
            ...
            >
              <PlusCircle
                size={22}
                className={pathname === "/dashboard" ? "animate-pulse" : ""}
              />
            </Link>
            <span className="text-[9px] font-bold text-cyan-600 dark:text-cyan-400 mt-0.5 uppercase tracking-wider">
              Groups
            </span>
          </div>
